...
$joomla(document).ready(function() {
   ...
   $joomla("input[name='declaredvalueTxt[]']").keypress(function (e) {
    ...
    $joomla('input[name="txtTracking"]').live('blur',function(e){
        ...
        var htms='';
        $joomla('#j_table tbody tr').each( function () {
        ...
        if($joomla(this).val()=='-'){
           $joomla.each($joomla('#j_table tbody tr#dfg'), function(){
            ...
            $joomla(this).closest('tr').after(htms);   // expand
...
<div class="container">
  <div class="main_panel persnl_panel">
    <div class="main_heading"><?php echo Jtext::_('COM_USERPROFILE_INVENTORY_TITLE');?></div>
          ...
          <div class="col-md-12">
            <div class="table-responsive">
              <table class="table table-bordered theme_table" id="j_table" data-page-length='10'>
                        <thead>
                           <tr> 
                             <th class="action_btns text-center">Actions</th>
                        ...
                        <tbody>
                            ...
                            foreach($ordersPendingView as $res){
                                ...
                                echo '<tr>
                                        		<td><button  class="expand_items btn btn-success" data-sno="item_wr_'.$i.'"  data-id="'.$res->BillFormNo.'">+</button></td>
                                ...
                                if(1){ // $rg->ItemQuantity>0
                                 ...
                                 // class="check-invisable"
                                       echo '<tr style="display:none;">
                                        		...
                                        		<td class="action_btns">';
                                        			if(1)
                                        		echo '<input type="checkbox" style="display:none;"  name="txtId" class="txtId selinpt-chksub" data-sno="item_wr_'.$i.'"  value="'.$rg->ItemName.':'.$res->BillFormNo.':'.$rg->ItemQuantity.':'.$res->TrackingId.':'.$rg->ItemIdk.':'.$rg->cost.':'.$rg->cost.':'.$idf.':'.$volres.':'.$res->ServiceType.':'.$res->Source.':'.$res->Dest_Cntry.':'.$res->MeasureUnits.':'.$res->Length.':'.$res->Width.':'.$res->Height.':'.$res->Weight.':'.$res->WeightUnit.':'.$sim.':'.$res->ShipmentType.':'.$res->SourceHub.':'.$res->DestinationCountryName.':'.$res->DestinationHubName.':'.$rg->Insurance.':'.$rg->ItemPrice.':'.$sim1.':'.$sim2.':'.$sim3.':'.$res->BusinessType.':'.$rg->Volume.':'.$rg->VolumetricWeight.'">
                                        		 <input type="button" style="display:none;" name="ship" class="ship" data-sno="'.$idf.'" data-id="'.$rg->ItemName.':'.$res->BillFormNo.':'.$rg->ItemQuantity.':'.$res->TrackingId.':'.$rg->ItemIdk.':'.$rg->cost.':'.$rg->cost.':'.$idf.':'.$volres.':'.$res->ServiceType.':'.$res->Source.':'.$res->Dest_Cntry.':'.$res->MeasureUnits.':'.$res->Length.':'.$res->Width.':'.$res->Height.':'.$res->Weight.':'.$res->WeightUnit.':'.$sim.':'.$res->ShipmentType.':'.$res->SourceHub.':'.$res->DestinationCountryName.':'.$res->DestinationHubName.':'.$rg->Insurance.':'.$rg->ItemPrice.':'.$sim1.':'.$sim2.':'.$sim3.':'.$res->BusinessType.':'.$rg->Volume.':'.$rg->VolumetricWeight.'" data-target="#ord_ship" title="'.Jtext::_('COM_USERPROFILE_SHIP_HISTORY_STATUS_SHIP').'">';
                                        			if(0)
